...
class MLP_TIME(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, time_dim, activation):
    ...
    def forward(self, x, t):
        ...
        time_emb = time_emb.unsqueeze(dim=1).repeat(1, n_gene, 1)
        h = h + time_emb
        return self.act(self.l2(h)) 
        # out2 = self.act(self.l2(h)) 
        # return self.l3(out2)
...
class DAZZLE_TIME(nn.Module):
    ...
    def __init__(
        ...
        self.inference_zposterior = MLP_TIME(1, hidden_dim, z_dim * 2, time_dim, activation)
        self.generative_pxz = MLP_TIME(z_dim, hidden_dim, 1, time_dim, activation)
        self.time_emb = SinusoidalPositionEmbeddings(time_dim)
        # self.da_classifier = MLP_TIME(z_dim, z_dim, 1, activation)
    ...
    def forward(self, x, t, global_mean, global_std, use_dropout_augmentation=True):                
        ...
        z_posterior = self.inference_zposterior(x.unsqueeze(-1), time_emb)
        ...
        x_rec = self.generative_pxz(z_inv, time_emb).squeeze(2)
