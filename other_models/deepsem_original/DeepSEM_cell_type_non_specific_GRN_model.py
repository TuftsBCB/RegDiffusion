...
from deepsem.Model import VAE_EAD
from deepsem.utils import evaluate, extractEdgesFromMatrix
...
class non_celltype_GRN_model:
    ...
    def init_data(self):
        ...
        data = sc.read(self.opt.data_file).transpose()
    ...
    def test(self):
        ...
        self.vae = VAE_EAD(adj_A_init, 1, opt.n_hidden, opt.K).float().cuda()
    ...
    def train_model(self):
        ...
        best_Epr = 0
        vae.train()
        for epoch in tqdm(range(opt.n_epochs + 1)):
        ...
        time_cost = str((end - start).total_seconds())
        np.save(opt.save_name+'_'+str(np.random.choice(100000))+'_'+time_cost, 
                vae.adj_A.cpu().detach().numpy())
