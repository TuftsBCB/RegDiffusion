...
class DAZZLE_EMB(nn.Module):
    ...
    def __init__(
        ...
        super(DAZZLE_EMB, self).__init__()
        self.n_gene = n_gene
        ...
        gene_emb = torch.rand(n_gene, hidden_dim)
        self.gene_emb = nn.Parameter(gene_emb, requires_grad=True)
        
        self.inference_zposterior = MLP(hidden_dim, hidden_dim, z_dim * 2, activation)
    ...
    def forward(self, x, global_mean, global_std, t, use_dropout_augmentation=True):  
        ...
        gene_emb = torch.einsum('cg,gd->cgd', x, self.gene_emb)
        z_posterior = self.inference_zposterior(gene_emb)
